...
describe('auth.service', () => {
  it('registers a user and returns token', async () => {
    ...
    const result = await registerUser({ email: 'a@b.com', password: 'secret', name: 'A' });
  ...
  it('rejects register when user exists', async () => {
    vi.mocked(userRepo.findUserByEmail).mockResolvedValueOnce({ id: 1 } as any);
  ...
  it('rejects login when user missing', async () => {
    vi.mocked(userRepo.findUserByEmail).mockResolvedValueOnce(null as any);
    await expect(loginUser({ email: 'a@b.com', password: 'secret' })).rejects.toMatchObject({
      status: 401,
    });
  });
});
